...
ProjectModel amber::project_model;

ProjectModel::ProjectModel(QObject *parent) : QAbstractItemModel(parent), root_item_(nullptr) { make_root(); }

ProjectModel::~ProjectModel() { destroy_root(); }
...
Media *ProjectModel::get_root() const { return root_item_.get(); }
...
QVariant ProjectModel::headerData(int section, Qt::Orientation orientation, int role) const {
  if (orientation == Qt::Horizontal && role == Qt::DisplayRole) return root_item_->data(section, role);
...
QModelIndex ProjectModel::create_index(int arow, int acolumn, void *adata) { return createIndex(arow, acolumn, adata); }
...
QModelIndex ProjectModel::parent(const QModelIndex &index) const {
  ...
  if (parentItem == get_root()) return QModelIndex();
...
bool ProjectModel::canFetchMore(const QModelIndex &parent) const {
  ...
  if (parent.isValid() && static_cast<Media *>(parent.internalPointer())->get_type() == MEDIA_TYPE_FOLDER) {
    return true;
  }
...
void ProjectModel::moveChild(MediaPtr child, Media *to) {
  ...
  beginMoveRows(from == get_root() ? QModelIndex() : createIndex(from->row(), 0, from), child->row(), child->row(),
                to == get_root() ? QModelIndex() : createIndex(to->row(), 0, to), to->childCount());
...
void ProjectModel::removeChild(Media *parent, Media *m) {
  ...
  beginRemoveRows(parent == get_root() ? QModelIndex() : createIndex(parent->row(), 0, parent), m->row(), m->row());
