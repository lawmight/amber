...
QString get_channel_layout_name(int channels, uint64_t layout) {
  switch (channels) {
    case 0:
      return QCoreApplication::translate("ChannelLayoutName", "Invalid");
    case 1:
      return QCoreApplication::translate("ChannelLayoutName", "Mono");
    case 2:
      return QCoreApplication::translate("ChannelLayoutName", "Stereo");
    default: {
      char buf[50];
...
Footage* Media::to_footage() { return static_cast<Footage*>(object.get()); }

SequencePtr Media::to_sequence() { return std::static_pointer_cast<Sequence>(object); }
...
void Media::set_icon(const QString& str) { set_icon(QIcon(str)); }

void Media::set_icon(const QIcon& ico) { icon = ico; }
  ...
  switch (type) {
    ...
    case MEDIA_TYPE_SEQUENCE: {
      ...
      tooltip = QCoreApplication::translate("Media",
                                            ...
                                            "\nAudio Frequency: %5"
                                            "\nAudio Layout: %6")
                    .arg(s->name, QString::number(s->width), QString::number(s->height), QString::number(s->frame_rate),
                         QString::number(s->audio_frequency), [&]() -> QString {
                           AVChannelLayout tmp = {};
...
VoidPtr Media::to_object() { return object; }

int Media::get_type() { return type; }

const QString& Media::get_name() {
  switch (type) {
    case MEDIA_TYPE_FOOTAGE:
      return to_footage()->name;
    case MEDIA_TYPE_SEQUENCE:
      return to_sequence()->name;
    default:
      return folder_name;
  }
}
...
int Media::get_sampling_rate(int stream) {
  switch (get_type()) {
    ...
    case MEDIA_TYPE_SEQUENCE:
      return to_sequence()->audio_frequency;
  }
  return 0;
}
...
Media* Media::child(int row) { return children.value(row).get(); }

int Media::childCount() const { return children.count(); }

int Media::columnCount() const { return 3; }
...
void Media::removeChild(int i) { children.removeAt(i); }
